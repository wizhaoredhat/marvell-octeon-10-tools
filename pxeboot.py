    ...
    def dpu_mac_ensure(
        ...
        *,
        ...
        def _on_missing() -> str:
            ...
            dpu_mac2, in_boot_menu = dpu_mac_detect(
                self,
...
def resolve_dpu_mac(dpu_mac: str, dpu_macs: Optional[dict[int, str]]) -> str:
    real_dpu_mac = netdev.validate_ethaddr_or_none(dpu_mac)
    if real_dpu_mac is not None:
        if dpu_macs is not None:
            if real_dpu_mac not in dpu_macs.values():
                raise RuntimeError(
                    f"Requested MAC {dpu_mac!r} was not found on DPU macs {dpu_macs}"
                )
        return real_dpu_mac

    if dpu_macs is None:
        raise RuntimeError(f"Cannot resolve MAC for {dpu_mac!r} without DPU macs")
    if not dpu_macs:
        raise RuntimeError(f"Cannot resolve MAC for {dpu_mac!r} with empty DPU macs")

    if dpu_mac == "primary":
        real_dpu_mac = dpu_macs[max(dpu_macs)]
    elif dpu_mac == "secondary":
        real_dpu_mac = dpu_macs[min(dpu_macs)]
    else:
        try:
            real_dpu_mac = dpu_macs[int(dpu_mac)]
        except (KeyError, ValueError):
            raise RuntimeError(
                f"Cannot find boot entry for {dpu_mac!r}. Detected interfaces are {dpu_macs}"
            )

    return netdev.validate_ethaddr(real_dpu_mac)
...
def dpu_mac_detect(
    ...
    *,
    ...
    if real_dpu_mac is not None:
        ...
        real_dpu_mac = resolve_dpu_mac(ctx.cfg.dpu_dev, dpu_macs)
